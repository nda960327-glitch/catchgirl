...
export default async function StaffLayout({ children, params }: { children: React.ReactNode; params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="mx-auto min-h-dvh w-full max-w-2xl bg-paper md:my-6 md:min-h-0 md:rounded-[28px] md:border-[6px] md:border-card md:shadow-pop">
      <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-line bg-card/90 px-5 py-3 backdrop-blur md:rounded-t-[22px]">
        ...
        <form action={logoutStaff.bind(null, slug)}><button className="rounded-full border border-line bg-card px-3 py-1.5 text-[10px] font-semibold text-mute">로그아웃</button></form>
      </header>
      <StaffNav slug={slug} />
