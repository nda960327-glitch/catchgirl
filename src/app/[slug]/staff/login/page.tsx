...
export default async function StaffLoginPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ next?: string }> }) {
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center bg-frame p-6">
      <div className="w-full max-w-sm rounded-[28px] border border-white bg-white/80 p-8 text-center shadow-pop backdrop-blur">
        ...
        <div className="font-serif text-[20px] font-bold text-ink">{store.name} 캐치걸 로그인</div>
        <StaffLoginForm slug={slug} next={next} />
        <div className="mt-6 text-[10px] text-mute">데모 — junhee / taeri / ain · 비밀번호 1234</div>
      </div>
    </main>
  );
}
